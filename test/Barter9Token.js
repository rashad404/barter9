...
contract('Barter9Token', function(accounts) {
    ...
    it('initializes the contract with the correct values', function(){
        return Barter9Token.deployed().then(function(instance) {
            ...
            assert.equal(name, 'Barter9', 'has the correct name');
            return tokenInstance.symbol();
        }).then(function(symbol){
            assert.equal(symbol, 'BTR9', 'has the correct symbol');
            return tokenInstance.standard();
        }).then(function(standard){
            assert.equal(standard, 'Barter9 v1.0', 'has the correct standard')
        })
    })
